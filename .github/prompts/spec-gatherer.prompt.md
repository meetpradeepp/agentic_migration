...
2. **"What edge cases should we handle?"** (empty data, invalid input, max limits, concurrent access)
3. **"What should happen when things go wrong?"** (failures, errors, timeouts, network issues)
4. **"What does success look like? How will you know it works?"**
5. **"Any constraints?"** (performance, compatibility, etc.)

Collect answers and document edge cases separately.
